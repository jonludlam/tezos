...
include Tz_log_core_lwt.Lwt_log_tz
...
let render ~buffer ~template ~section ~level ~message =
  ...
  Buffer.add_substitute buffer
    (function
      ...
      | "message" -> message
      | "level" -> Tz_log_core.Log_core.string_of_level level
      | "section" -> Tz_log_core.Log_core.Section.name section
      | "loc-file" -> file
...
let _ = Tz_log_core_lwt.Lwt_log_tz.default := channel ~close_mode:`Keep ~channel:Lwt_io.stderr ()
